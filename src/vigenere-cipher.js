  ...
  createResult(msg, str, nameFunc) {
    ...
    for (let i = 0; i < msg.length; i++) {
      if (!this.alphabet.includes(msg[i])) {
        ...
        switch (nameFunc) {
          case 'encrypt':
            index = this.alphabet.indexOf(msg[i]) + this.alphabet.indexOf(str[i]);
            if (index > this.alphabet.length) index -= this.alphabet.length;
            if (index === this.alphabet.length) index = 0;
            break;
          case 'decrypt':
            index = (this.alphabet.length - this.alphabet.indexOf(str[i])) + this.alphabet.indexOf(msg[i]);
            if (index > this.alphabet.length) index -= this.alphabet.length;
            if (index === this.alphabet.length) index = 0;
            break;
        }
    ...
    return this.bool === true ? result.toUpperCase() : this.reverseResult(result);
  }
}
